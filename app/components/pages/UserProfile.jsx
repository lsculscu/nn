...
export default class UserProfile extends React.Component {
    ...
    render() {
        ...
        if( section === 'transfers' ) {
           ...
           const reply_summary1 = account.recent_replies.map( item => {
               return (<li style={{listStyleType: 'none'}} key={item}>
                   <PostSummary post={item} currentCategory="-" />
               </li>);
            });
            const reply_summary = <div className="bg2">{reply_summary1}</div>;
            tab_content = reply_summary1.length ? reply_summary :
