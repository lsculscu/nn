...
class Topics extends React.Component {
    ...
    render() {
        ...
        categories = categories.map(cat => {
            ...
            return (<li className= "above" key={cat}>
                        <Link to={link} activeClassName="active">{cat}</Link>
                    </li>);
        });
        return (
            <ul className={cn}>
                <li className="Topics__title above" key={'*'}>Tags and Topics</li>
                <li className="Topics__filter above"><input type="text" placeholder="Filter" value={search} onChange={onChangeSearch} /></li>
               {categories}
